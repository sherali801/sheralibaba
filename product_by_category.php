...
if ($result) {
  ...
  $sql = "SELECT COUNT(*) FROM product WHERE (visibility = 1) AND (category_id = {$category_id})";
  if (!empty($q)) {
      ...
      <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
        ...
        <form class="navbar-form navbar-left" action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="get">
          <div class="form-group">
            <input type="hidden" name="category_id" value="<?php echo urlencode($category_id); ?>">
...
<div class="container container-fluid text-center">
  <?php if ($total_count > 0) { ?>
    <h2><?php echo htmlentities($category_name); ?> Products</h2>
    <div class = "row">
    ...
    <ul class="pagination">
      ...
      if($total_pages > 1) {
        if($current_page - 1 >= 1) {
          ...
          echo "<li><a href='product_by_category.php?category_id={$category_id}&q=". urlencode($q) . "&current_page={$pre_page}'>&laquo;</a></li>";
        }
        for($i=1; $i <= $total_pages; $i++) {
          if ($i == $current_page) {
            echo "<li class='active'><a href='product_by_category.php?category_id={$category_id}&q=". urlencode($q) . "&current_page={$i}'>{$i}</a></li>";
          } else {
            echo "<li><a href='product_by_category.php?category_id={$category_id}&q=". urlencode($q) . "&current_page={$i}'>{$i}</a></li>";
          }
        }
        if(($current_page + 1) <= $total_pages) {
          $next_page = $current_page + 1;
          echo "<li><a href='product_by_category.php?category_id={$category_id}&q=". urlencode($q) . "&current_page={$next_page}'>&raquo;</a></li>";
        }
      }
      ?>
    </ul>
